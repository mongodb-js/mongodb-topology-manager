  ...
  start() {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        yield Promise.map(self.shards, shard => shard.purge());

        // Then boot up the shards
        yield Promise.map(self.shards, function(shard) {
          if (self.logger.isInfo()) {
        ...
        yield Promise.map(self.proxies, proxy => proxy.purge());

        // Then boot up the proxies
        yield Promise.map(self.proxies, function(proxy) {
          if (self.logger.isInfo()) {
  ...
  stop() {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        yield Promise.map(self.proxies, proxy => proxy.stop());
        ...
        yield Promise.map(self.shards, shard => shard.stop());
