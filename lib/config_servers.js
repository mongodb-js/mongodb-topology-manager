...
class ConfigServers extends EventEmitter {
  ...
  discover() {
    ...
    return new Promise(function(resolve, reject) {
      ...
      proc.on('close', function() {
        ...
        var sslMatch = stdout.match(/ssl/i) || stderr.match(/ssl/);
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        const secondaries = yield Promise.map(self.managers, server => server.ismaster()).reduce(
          (result, ismaster, idx) => {
            if (ismaster.secondary) result.push(self.managers[idx]);
            return result;
          },
          []
        );
  ...
  stepDownPrimary(returnImmediately, options, credentials) {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        try {
          yield manager.executeCommand('admin.$cmd', command, credentials);
        } catch (err) {
  ...
  configuration(manager, credentials) {
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        var result = yield manager.executeCommand(
          'admin.$cmd',
          {
  ...
  stop() {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        yield Promise.map(self.managers, server => server.stop());
