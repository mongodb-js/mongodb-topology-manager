  ...
  removeMember(node, options, credentials) {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        // } catch(err) {
        //     console.log("---------------------------- removeMember ## 1-1")
        //     console.dir(err)
        // }

            // console.log("---------------------------- removeMember ## 2")

        // if(result && result.ok == 0) {
        //   return reject(new Error(f('failed to execute replSetReconfig with configuration [%s]', JSON.stringify(config))))
        // }

            // console.log("---------------------------- removeMember ## 3")
        ...
        while(true) {
          try {
            ...
            // console.log("---------------------------- removeMember 5")
              // Wait for a primary to appear
              yield self.waitForPrimary();
            // console.log("---------------------------- removeMember 6")
              // Resolve
              return resolve();
            } else {
            // console.log("---------------------------- removeMember 7")
              yield waitMS(self.retryWaitMS);
            }
          } catch(err) {
            // console.log("---------------------------- removeMember 8")
            yield waitMS(self.retryWaitMS);
          }
        }
