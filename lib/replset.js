...
class ReplSet extends EventEmitter {
  constructor(binary, nodes, options) {
    ...
    // Wait times
    this.electionCycleWaitMS =
      typeof this.options.electionCycleWaitMS === 'number'
        ? this.options.electionCycleWaitMS
        : 31000;
    this.retryWaitMS =
      typeof this.options.retryWaitMS === 'number' ? this.options.retryWaitMS : 5000;
  ...
  start() {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        // Use the given protocol version or the minimum required. MongoDB 4.0+ requires protocol version 1
        var result = yield self.discover();
        var protocolVersion = self.options.protocolVersion || result.version[0] >= 4 ? 1 : 0;

        // Boot all the servers
        yield self.managers.map(function(server) {
          return server.start();
        });
        ...
        // Ignore Error
        var ignoreError = result.version[0] === 2 && result.version[1] <= 6 ? true : false;

        // Pick the first manager and execute replicaset configuration
        result = yield self.managers[0].executeCommand(
          'admin.$cmd',
          {
        ...
        if (result.ok === 0) {
          return reject(
            new Error(f('failed to initialize replicaset with config %s', JSON.stringify(config)))
          );
        }
        ...
        while (true) {
          ...
          try {
            result = yield self.managers[0].executeCommand('admin.$cmd', {
  ...
  arbiters() {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        // Call ismaster on all of the managers first (in parallel)
        var ismasters = yield self.managers.map(function(server) {
          return server.ismaster();
        });

        // Then filter and remove any non arbiters
        var arbiters = ismasters
          .map(function(ismaster, i) {
            return ismaster.arbiterOnly ? self.managers[i] : null;
          })
          .filter(x => x);
  ...
  secondaries() {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        // Then filter and retrieve secondaries
        var secondaries = ismasters
          .map(function(ismaster, i) {
            return ismaster.secondary &&
              (!ismaster.passives || ismaster.passives.indexOf(ismaster.me) === -1)
              ? self.managers[i]
              : null;
          })
          .filter(x => x);
  ...
  passives() {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        // Then filter and retrieve passive secondaries
        var secondaries = ismasters
          .map(function(ismaster, i) {
            return ismaster.secondary &&
              ismaster.passives &&
          ...
          for (var i = 0; i < self.managers.length; i++) {
            try {
              ...
              if (
                ...
                // Return the manager
                return resolve(self.managers[i]);
              } else if (ismaster.ismaster && !waitedForElectionCycle) {
                // Wait for 31 seconds to allow a full election cycle to pass
                yield delay(self.electionCycleWaitMS);
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        if (result[0] >= 3) {
          ...
          result = yield manager.executeCommand(
            'admin.$cmd',
            {
          ...
          if (result && result.ok === 0) {
            return reject(
              new Error(f('failed to execute replSetGetConfig against server [%s]', manager.name))
            );
          }
  ...
  reconfigure(config, options, credentials) {
    ...
    var returnImmediately =
      typeof options.returnImmediately === 'boolean' ? options.returnImmediately : false;
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        // Execute the reconfigure command
        var result = yield primary.executeCommand(
          'admin.$cmd',
          {
        ...
        if (result && result.ok === 0) {
          return reject(
            new Error(
              f('failed to execute replSetReconfig with configuration [%s]', JSON.stringify(config))
            )
          );
        }
        ...
        if (returnImmediately) return resolve();
        ...
        while (true) {
          try {
            let primary = yield self.primary();
            if (!primary) {
              yield delay(self.retryWaitMS);
              continue;
            }
            ...
            if (
              ...
              (ismaster.ismaster || ismaster.secondary || ismaster.arbiterOnly) &&
  ...
  addMember(node, options, credentials) {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        for (var i = 0; i < self.managers.length; i++) {
          if (
            f('%s:%s', self.managers[i].host, self.managers[i].port) ===
            f('%s:%s', server.host, server.port)
          ) {
  ...
  removeMember(node, options, credentials) {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        if (!result.ok) {
          return reject(new Error('reconfigure failed'));
        }
  ...
  maintenance(value, node, options, credentials) {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        var result = yield node.executeCommand(
          'admin.$cmd',
          {
        ...
        if (result && result.ok === 0) {
          return reject(
            new Error(f('failed to execute replSetMaintenance for server [%s]', node.name))
          );
        }
        ...
        while (true) {
          if (currentTries === 0) {
            return reject(
              new Error(
                f(
                  'server %s failed to come back as a secondary after %s milliseconds waiting',
                  node.name,
                  maxRetries * 1000
                )
              )
