...
class ReplSet extends EventEmitter {
  ...
  arbiters() {
    ...
    return Promise.map(self.managers, server => server.ismaster()).reduce(
      (result, ismaster, idx) => {
  ...
  stop(signal) {
    ...
    return Promise.map(self.managers, server => server.stop(signal));
  }
  ...
  purge() {
    ...
    return Promise.map(self.managers, server => server.purge());
  }
}
