  ...
  start() {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        var result = yield self.managers[0].executeCommand(
          'admin.$cmd',
          {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        if (result[0] >= 3) {
          ...
          if (result && result.ok == 0) {
            return reject(
              new Error(f('failed to execute replSetGetConfig against server [%s]', node.name))
            );
          }
  ...
  reconfigure(config, options, credentials) {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        reject(
          new Error(f('failed to successfully set a configuration [%s]', JSON.stringify(config)))
        );
      }).catch(reject);
    });
  }
  ...
  addMember(node, options, credentials) {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        reject(
          new Error(
            f('failed to successfully add a new member with options [%s]', JSON.stringify(node))
          )
        );
      }).catch(reject);
    });
