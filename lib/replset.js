...
class ReplSet extends EventEmitter {
  ...
  start() {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        yield Promise.map(self.managers, server => server.start());
  ...
  arbiters() {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        const arbiters = yield Promise.map(self.managers, server => server.ismaster()).reduce(
          (result, ismaster, idx) => {
            if (ismaster.arbiterOnly) result.push(self.managers[idx]);
            return result;
          },
          []
        );
  ...
  secondaries() {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        const secondaries = yield Promise.map(self.managers, server => server.ismaster()).reduce(
          (result, ismaster, idx) => {
            if (
              ismaster.secondary &&
              (!ismaster.passives || ismaster.passives.indexOf(ismaster.me) === -1)
            ) {
              result.push(self.managers[idx]);
            }
  ...
  stop(signal) {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        yield Promise.map(self.managers, server => server.stop(signal));
  ...
  purge() {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        yield Promise.map(self.managers, server => server.purge());
