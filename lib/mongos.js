...
class Mongos extends EventEmitter {
   ...
   * @param {number} [options.connectionTimeoutMS=5000] TCP connection timeout setting in ms
   * @param {number} [options.socketTimeoutMS=0] TCP socket timeout setting in ms
  ...
  executeCommand(ns, command, credentials, options) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      opt.connectionTimeout = options.connectionTimeoutMS || 5000;
      opt.socketTimeout = options.socketTimeoutMS || 0;
      opt.pool = 1;
  ...
  /**
   * Retrieve document that describes the role of the mongos instance
   *
