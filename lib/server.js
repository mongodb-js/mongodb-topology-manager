...
class Server {  
  ...
  ssl(serverOptions, clientOptions) {
    serverOptions = serverOptions || {};
    clientOptions = clientOptions || {};

    // Merge the options together
    for(var name in clientOptions) {
      this.clientOptions[name] = clientOptions[name];
    }

    // Merge the server options together
    for(var name in serverOptions) {
      this.serverOptions[name] = serverOptions[name];
    }
  }
  ...
  start() {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        for(var name in self.serverOptions) {
          options[name] = self.serverOptions[name];
        }
