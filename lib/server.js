...
class Server extends EventEmitter {
  ...
  instance(credentials, options) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      var opt = Object.assign({}, clone(self.clientOptions), options);
      opt.host = self.options.bind_ip;
      opt.port = self.options.port;
      opt.connectionTimeout = options.connectionTimeout || 5000;
      opt.socketTimeout = options.socketTimeout || 0;
      opt.pool = 1;
  ...
  start() {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        // Periodically attempt to connect to server and return if successful
        // Error will be thrown if unable to connect after certain number of retries
        yield waitForAvailable(self.options.bind_ip, self.options.port);
